...
class ReviewsSlider extends React.Component {
  ...
  render() {
    let marginStyle = `-${this.state.currentPosition * 100}%`;
    const { t } = this.props;

    return (
      <div className="intro-slider-wrapper">
        <button className="slider-paginator prev" onClick={this.movePrev}></button>
        <button className="slider-paginator next" onClick={this.moveNext}></button>

        <div className="intro-slider" onMouseEnter={() => { this.setState({ sliderPaused: true }) }} onMouseLeave={() => { this.setState({ sliderPaused: false }) }}>
          <ul className="slides">
